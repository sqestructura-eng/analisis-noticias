...
# ═══════════════════════════════════════════════════════════════════════════
# ── CALENDARIO ECONÓMICO (ForexFactory) — módulo independiente ─────────────
# No modifica ni interactúa con la lógica de análisis de noticias/Groq.
# Fuente pública gratuita, sin API key: nfs.faireconomy.media
# ═══════════════════════════════════════════════════════════════════════════

FF_CALENDAR_URL = "https://nfs.faireconomy.media/ff_calendar_thisweek.json"

# Divisas que te interesan (coinciden con tus pares operados)
CALENDAR_CURRENCIES = ["USD", "GBP", "JPY", "EUR"]

# Solo eventos de máximo impacto (equivalente a las 3 estrellas de Investing.com)
CALENDAR_HIGH_IMPACT = "High"

seen_calendar_events = set()


def fetch_economic_calendar() -> list:
    """Descarga el calendario semanal de ForexFactory y filtra eventos
    de impacto ALTO (3 estrellas) para USD, GBP y JPY."""
    headers = {
        "User-Agent": (
    ...
    try:
        r = requests.get(FF_CALENDAR_URL, headers=headers, timeout=15)
        r.raise_for_status()
        events = r.json()
    except Exception as e:
        log.error(f"Error obteniendo calendario economico: {e}")
        return []
    ...
    for event in events:
        try:
            ...
            if currency not in CALENDAR_CURRENCIES:
                continue
            if impact != CALENDAR_HIGH_IMPACT:
                continue
            ...
            # Solo eventos que ya se publicaron (tienen "actual") en la última hora,
            # para no repetir alertas de eventos futuros todavía sin dato.
            actual = event.get("actual", "")
            if not actual:
                continue

            window_start = now.replace(minute=0, second=0, microsecond=0)
            event_window = event_dt.replace(minute=0, second=0, microsecond=0)
            if event_window != window_start:
                continue

            event_id = f"{currency}_{event.get('title','')}_{event_date_str}"
            if event_id in seen_calendar_events:
                continue
            seen_calendar_events.add(event_id)
            ...
            log.error(f"Error procesando evento del calendario: {e}")
            continue

    return filtered


def send_calendar_alert(events: list) -> None:
    """Envía a Telegram un resumen de los eventos de 3 estrellas detectados."""
    if not events:
        return

    timestamp = datetime.now(timezone.utc).strftime("%d/%m/%Y %H:%M UTC")
    lines = [f"📅 *CALENDARIO ECONÓMICO — {timestamp}*", ""]
    lines.append("Datos de alto impacto (3★) recién publicados:\n")

    for ev in events:
        hora = ev["date"].strftime("%H:%M UTC")
        lines.append(f"• [{ev['currency']}] {ev['title']} — {hora}")
        lines.append(f"   Actual: {ev['actual']}  |  Forecast: {ev['forecast']}  |  Previo: {ev['previous']}")
        lines.append("")
        log.info(f"[CALENDARIO] Impacto ALTO - {ev['currency']} | {ev['title']} | Actual: {ev['actual']}")

    message = "\n".join(lines)
    send_telegram(message)
    log.info("Alerta de calendario economico enviada a Telegram.")


def check_economic_calendar() -> None:
    """Punto de entrada del chequeo horario del calendario economico."""
    log.info("Revisando calendario economico (USD/GBP/JPY, 3 estrellas)...")
    events = fetch_economic_calendar()
    if events:
        send_calendar_alert(events)
    else:
        log.info("Sin eventos de alto impacto nuevos en el calendario economico.")

# ═══════════════════════════════════════════════════════════════════════════
# ── FIN módulo calendario económico ─────────────────────────────────────────
# ═══════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    schedule.every(CHECK_INTERVAL_HOURS).hours.do(check_economic_calendar)
